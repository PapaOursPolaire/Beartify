...
fn update_media_session(
    ...
    // ⚠️ CORRECTIF : sur Android, MediaPlaybackService (foreground service
    // Kotlin, voir onMediaSessionUpdate côté JS) poste déjà la vraie
    // notification MediaStyle avec contrôles/pochette/couleur dynamique.
    // Poster aussi cette notification "fallback" ici faisait apparaître
    // une seconde notification basique et redondante ("notif test") en
    // plus de la vraie. Cette fonction ne reste donc active que sur
    // desktop, où il n'existe pas de foreground service équivalent.
...
// ══════════════════════════════════════════════════════════════════
//  Échange de tokens Google (OAuth PKCE + refresh) — fait côté Rust.
//
//  Pourquoi : le fetch() natif du WebView (et même window.__TAURI__.http,
//  dont l'espace de noms JS n'est apparemment pas exposé dans ce build
//  sans bundler) vers https://oauth2.googleapis.com/token échouait
//  silencieusement sur Linux (WebKitGTK) — CORS bloqué, sans erreur
//  visible côté JS. Faire la requête ici, côté Rust, élimine le problème :
//  aucun CORS ne s'applique à une requête HTTP faite par le processus
//  natif, seulement aux requêtes émises depuis le contexte du WebView.
//
//  `params` est directement le corps x-www-form-urlencoded déjà construit
//  côté JS (mêmes clés que celles envoyées à Google : client_id, code,
//  code_verifier, grant_type, redirect_uri, client_secret, ou bien
//  refresh_token pour le flux de restauration silencieuse).
// ══════════════════════════════════════════════════════════════════
#[tauri::command]
async fn google_token_exchange(params: std::collections::HashMap<String, String>) -> Result<serde_json::Value, String> {
    eprintln!("[MARQUEUR_BUILD_UNIQUE_84729] google_token_exchange invoquée");
    eprintln!("[DEBUG google_token_exchange] Commande invoquée, params reçus : {} clés", params.len());

    let client = reqwest::Client::builder()
    .timeout(std::time::Duration::from_secs(15))
    .build()
    .map_err(|e| { eprintln!("[DEBUG google_token_exchange] Échec build client : {e}"); format!("Impossible de créer le client HTTP : {e}") })?;

    eprintln!("[DEBUG google_token_exchange] Client HTTP créé, envoi de la requête POST vers Google...");
    ...
    .send()
    .await
    .map_err(|e| { eprintln!("[DEBUG google_token_exchange] Échec de send().await : {e}"); format!("Requête vers Google échouée : {e}") })?;

    eprintln!("[DEBUG google_token_exchange] Réponse reçue de Google, status = {}", res.status());

    let status = res.status();
    let body: serde_json::Value = res
    .json()
    .await
    .map_err(|e| format!("Réponse Google illisible (status {status}) : {e}"))?;

    if !status.is_success() {
        return Err(format!("Google a répondu {status} : {body}"));
    }
...
pub fn run() {
    ...
    // ⚠️ AJOUTÉ : manquait entièrement — c'est la cause de l'erreur
    // "Command plugin:dialog|open not allowed by ACL". Un plugin absent
    // du binaire renvoie toujours cette erreur, même avec la permission
    // correspondante dans default.json (celle-ci vient d'être ajoutée
    // aussi, voir default.json — les deux étaient nécessaires).
    // Nécessite la dépendance dans Cargo.toml : tauri-plugin-dialog = "2"
    .plugin(tauri_plugin_dialog::init())
    // ⚠️ AJOUTÉ : requis par le nouveau bouton "Quitter" du pop-up
    // hors-ligne (local-library.js appelle plugin:process|exit).
    // Nécessite la dépendance dans Cargo.toml : tauri-plugin-process = "2"
