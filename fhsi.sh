#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════
#  Correctif définitif : les handlers routerAdd() de PocketBase sont
#  exécutés comme des "programmes" sérialisés isolés — ils n'ont PAS
#  accès aux const/functions déclarées en dehors d'eux, même dans le
#  même fichier .pb.js (limitation documentée de la JSVM PocketBase).
#  D'où le "ReferenceError: FIREBASE_WEB_API_KEY is not defined".
#
#  Solution officielle : déplacer le code partagé dans un module JS
#  séparé (pas de suffixe .pb.js pour qu'il ne soit pas auto-chargé
#  comme hook) et le charger via require() À L'INTÉRIEUR de chaque
#  handler.
#
#  ⚠️ Renseigne FIREBASE_WEB_API_KEY avant de lancer.
# ══════════════════════════════════════════════════════════════════
set -euo pipefail
...
# ── 1. Module partagé (PAS de suffixe .pb.js — chargé via require(), pas auto-hook) ──
sudo tee /opt/pocketbase/pb_hooks/bridge_utils.js > /dev/null << UTILS_EOF
module.exports = {
  SERVER_BRIDGE_SECRET: "${SERVER_BRIDGE_SECRET}",
  FIREBASE_WEB_API_KEY: "${FIREBASE_WEB_API_KEY}",
  PB_BASE_URL: "http://127.0.0.1:${PB_PORT}",
};

module.exports.derivePassword = function (externalId) {
  return \$security.hs256(externalId, module.exports.SERVER_BRIDGE_SECRET);
};

module.exports.upsertUserAndGetSession = function (provider, externalId, displayName, avatarUrl) {
  const collection = \$app.findCollectionByNameOrId("users");
  const password = module.exports.derivePassword(externalId);
  ...
  \$app.save(record);
  const res = \$http.send({
    url: module.exports.PB_BASE_URL + "/api/collections/users/auth-with-password",
...
# ── 2. Hook principal : require() le module À L'INTÉRIEUR de chaque handler ──
...
sudo chown pocketbase:pocketbase /opt/pocketbase/pb_hooks/bridge_utils.js /opt/pocketbase/pb_hooks/beartify_auth_bridge.pb.js
sudo chmod 640 /opt/pocketbase/pb_hooks/bridge_utils.js /opt/pocketbase/pb_hooks/beartify_auth_bridge.pb.js
sudo sed -i 's/\r$//' /opt/pocketbase/pb_hooks/bridge_utils.js /opt/pocketbase/pb_hooks/beartify_auth_bridge.pb.js

echo "→ Redémarrage complet du service (kill + start, pas juste restart)..."
sudo systemctl stop pocketbase-beartify
sudo pkill -9 -f "opt/pocketbase/pocketbase" 2>/dev/null || true
sleep 1
sudo systemctl start pocketbase-beartify
sleep 2

echo "→ Test en local avec un faux token (doit renvoyer 'Jeton Firebase invalide', pas une ReferenceError)..."
curl -s -X POST "http://127.0.0.1:${PB_PORT}/api/beartify/session/google" \
  -H "Content-Type: application/json" -d '{"idToken":"faux_token_de_test"}'
echo ""
