...
//       + fix v15 : queue de téléchargement sérialisée (800ms entre chaque) — CEF Linux bloque les clics simultanés
...
(function () {
  ...
  const state = {
    ...
    dlQueue        : [],   // file de téléchargements sérialisés (Linux CEF : 1 clic à la fois)
    dlBusy         : false,
  };
  ...
  /* ═══════════════════════════════════════════════════════════
     TÉLÉCHARGEMENT SÉRIALISÉ
     CEF Linux bloque les a.click() programmatiques successifs
     s'ils arrivent trop vite. On les sérialise avec 800ms d'écart.
  ═══════════════════════════════════════════════════════════ */
  const DL_INTERVAL = 800; // ms entre deux téléchargements

  function triggerDownload(blob, filename) {
    state.dlQueue.push({ blob, filename });
    if (!state.dlBusy) processDlQueue();
  }

  function processDlQueue() {
    if (state.dlQueue.length === 0) { state.dlBusy = false; return; }
    state.dlBusy = true;
    const { blob, filename } = state.dlQueue.shift();
    ...
    setTimeout(processDlQueue, DL_INTERVAL);
  }
