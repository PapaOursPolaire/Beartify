...
get_user_preferences() {
    ...
    if [[ "$INSTALL_MONITORING" =~ ^[Yy] ]]; then
        GRAFANA_PASS=$(grep admin_password /etc/grafana/grafana.ini | cut -d'=' -f2 | tr -d ' ')
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    # Création des outils de maintenance
    create_maintenance_tools
...
server {
    ...
    # Redirection HTTPS si domaine configuré
    $(if [[ -n "$DOMAIN" ]]; then echo "return 301 https://\$server_name\$request_uri;"; else echo "# HTTP seulement"; fi)
    
    $(if [[ -z "$DOMAIN" ]]; then cat <<'HTTPCONF'
    location / {
...
[security]
admin_user = admin
admin_password = beartify_admin_$(openssl rand -base64 8)
...
EOF
        ...
        sudo tee /var/lib/grafana/dashboards/beartify-overview.json > /dev/null <<'EOF'
{
  "dashboard": {
